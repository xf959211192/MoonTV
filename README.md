...
| 变量                                | 说明                                         | 可选值                           | 默认值                                                                                                                     |
| ----------------------------------- | -------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| REMOTE_CONFIG_URL                   | 远程配置文件 URL，用于订阅外部配置           | URL                              | (空)                                                                                                                       |
...
除了本地 `config.json`，本项目还支持通过 URL 订阅远程配置文件。这允许你动态更新资源站点，而无需修改代码或重新部署。

- **工作方式**：
  1.  在环境变量中设置 `REMOTE_CONFIG_URL`，指向你的远程配置文件地址。
  2.  应用启动时会优先尝试从该 URL 拉取配置。
  3.  如果远程配置拉取失败，应用会回退使用本地的 `config.json` 文件。
  4.  远程配置支持普通 JSON 或 Base58 编码的 JSON。
  5.  应用会每 24 小时自动热更新一次远程配置。

- **环境变量**：
  - `REMOTE_CONFIG_URL`：你的远程配置文件的 URL。例如 `https://example.com/my-config.json`。
